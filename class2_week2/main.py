...
"""
def compare_gradient_descent_method():
...
def random_mini_batches(X, Y, mini_batch_size=64, seed=0):
    ...
    mini_batches = []
    # Step 1: Shuffle (X, Y)
    permutation = list(np.random.permutation(m))    # 随机数组
    shuffled_X = X[:, permutation]
    shuffled_Y = Y[:, permutation].reshape((1, m))

    # Step 2: Partition
    num_complete_minibatches = math.floor(m/mini_batch_size)   # 向下舍入到最接近的整数
...
def initialize_velocity(parameters):
    ...
    for l in range(L):
        ...
        v['db' + str(l + 1)] = np.zeros((bl.shape[0], bl.shape[1]))
    return v
...
def initialize_adam(parameters):
    ...
    for l in range(L):
        ...
        s['db' + str(l + 1)] = np.zeros((bl.shape[0], bl.shape[1]))
    return v, s
